...
if ($pass_len<6) {
    ...
    header('location: user_settings.php?m3=' . $m);
} else if ($new_pass != $new_pass1) {
    header('location: user_settings.php?error=The two passwords don\'t match');
} else {
    if ($old_pass == $orig_pass) {
        $query = "UPDATE  user SET password = '" . $new_pass . "' WHERE email = '" . $_SESSION['email'] . "'";
        mysqli_query($con, $query) or die($mysqli_error($con));
        header('location: user_settings.php?error=Password Updated');
    } else
        header('location: user_settings.php?error=Wrong Old Password');
}
?>